...
| `ontos` | discrete multiscale (life v1) + nbody gravity with Chebyshev ephemeris windows, zoom policy (v2) | independent spec reimplementation | stream records vs reference, bit-exact state hashes, bounded window drift, zoom-policy conformance, REBOUND anchor (<1e-4) |
